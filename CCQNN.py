...
emp_train_X = X_data_emp[:t_index,:]
emp_train_Y = Y_data_emp[:t_index,:]
emp_test_X = X_data_emp[t_index:,:]
emp_test_Y = Y_data_emp[t_index:,:]

ver_train_X = X_data_ver[:t_index,:]
ver_train_Y = Y_data_ver[:t_index,:]
ver_test_X = X_data_ver[t_index:,:]
ver_test_Y = Y_data_ver[t_index:,:]
######################## data prep ###########################################
...
emp_hist = emp_model.fit(emp_train_X, emp_train_Y, 
                epochs=EPOCHS, batch_size=500,
                verbose=1, validation_data=(emp_test_X, emp_test_Y)
                )
#save the model
emp_model.save('emp_model.h5') #creates a hdf5 file

# evaluate the empirical value model
loss_and_metrics = emp_model.evaluate(emp_test_X, emp_test_Y, batch_size=50)
print("\n%s: %.2f%%" % (emp_model.metrics_names[1], loss_and_metrics[1]*100))

#val loss graph
loss = emp_hist.history['loss']
val_loss = emp_hist.history['val_loss']
plt.plot(loss)
plt.plot(val_loss)
plt.legend(['loss', 'val_loss'])
plt.show()
# draw the loss graph
acc = emp_hist.history['acc']
val_acc = emp_hist.history['val_acc']
plt.plot(acc)
plt.plot(val_acc)
